<template>
  <div class="content">
    <div
      class="header"
      v-if="$store.getters.getStateHead"
      ref="header"
      :style="
        $store.getters.getStateType
    ...
    >
      <div class="header-logo" @click="onIndex">
        <img
          v-show="$store.getters.getStateType"
      ...
      <div
        :class="
          $store.getters.getStateType ? 'header-menu menu-white' : 'header-menu'
        "
        @click="onToggle"
      >
        <span></span>
    ...
    <div
      ...
      :style="[
        $store.getters.getStateType
          ? 'height:' + clientHeight + 'px;margin: 0;'
          : 'height:' + panelHeight + 'px;',
        $store.getters.getStateHead ? '' : 'margin: 0;',
      ]"
    >
      <slot></slot>
    </div>
  ...
  mounted() {
    ...
    if (advert == process.env.VUE_APP_VERSION) {
      ...
      if (
        !(
          ...
          location.href.includes("mitax") ||
          location.href.includes("email")
        )
      ) {
        // setTimeout(() => {
    ...
    let giftTimer = setInterval(() => {
      if (!this.$store.getters.getStateGiftSend) {
        ...
        if (
          nowTime >= getCookie("expiredTime") &&
          !location.href.includes("error") &&
          !location.href.includes("email") &&
          !this.$store.getters.getStateGiftbag
        ) {
